...
def get_raw_datalake_url(source: Source, version: str) -> str:
    prefix = get_raw_datalake_prefix(source, version)
    return f"s3://{raw_bucket_name}/{prefix}"


def get_normalized_datalake_url(source: Source, version: str) -> str:
    prefix = get_normalized_datalake_prefix(source, version)
